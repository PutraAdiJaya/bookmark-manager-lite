...
import { collapseAllCommand, expandAllCommand } from './commands/collapseExpandAll';

export async function activate(context: vscode.ExtensionContext) {
  // Get workspace root
  const workspaceFolders = vscode.workspace.workspaceFolders;
  if (!workspaceFolders || workspaceFolders.length === 0) {
    vscode.window.showWarningMessage('Bookmark Manager Lite requires a workspace to be opened');
    return;
  }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('bookmarkLite.open', (bookmark) => openBookmarkCommand(bookmarkService, bookmark))
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('bookmarkLite.editTags', (bookmark) => editBookmarkTagsCommand(bookmarkService, bookmark))
  );
